...
This section describes how to run the SDK tests. If you want to run the tests of a specific project, go to that project's folder and execute `pnpm test`. In packages that support both runtimes, that script typically runs both `test:node` and `test:browser`; some packages intentionally skip one environment. To target these environments individually, run the package's `pnpm test:node` and `pnpm test:browser` scripts if they exist. Let's take `purview-datamap-rest` as an example.
...
Before running tests, install dependencies and build the package from the repo root with `pnpm install && pnpm turbo build --filter=<package-name>... --token 1`. This can take several minutes. For more details, see the [development workflow guidance](https://github.com/Azure/azure-sdk-for-js/blob/main/CONTRIBUTING.md#development-workflows).
