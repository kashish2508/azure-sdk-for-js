...
1. **Create a swagger/README.md file under ${PROJECT_ROOT} folder**
    ...
    **After the first generation, set `generate-metadata: false` if you make manual edits to generated metadata files and need to preserve them across regeneration.**
...
In order to release your RLC library, we need to add some tests for it to make sure we are delivering high quality packages. But before adding the test, we need to enable the option `generate-test: true` to generate the necessary change in `package.json` and `tsconfig.json` so that test framework can work. Once the generation finished, you will see a `sample test file (for example `sampleTest.spec.ts`)` file in your `{PROJECT_ROOT}/test/public` folder, which has an empty test and you could add/update test cases against your own services.
...
3. **Push recording to assets repo**
    ...
    `Notice`:
      - the tests have to be recorded using the `TEST_MODE=record`, then the recording files will be generated.
...
Please ensure that your code changes and `assets.json` updates are committed together. Recordings themselves live in `Azure/azure-sdk-assets`.
