...
2. **Run the test**
    ...
    export TEST_MODE=record && pnpm test # this will run live tests and generate recordings that you then push via asset sync.
    ```
    ...
    ```shell
    pnpm turbo build --filter=${PACKAGE_NAME}... --token 1
    SET TEST_MODE=record&& pnpm test # this will run live tests and generate recordings that you then push via asset sync.
    ```
    ...
    ```shell
      pnpm turbo build --filter=${PACKAGE_NAME}... --token 1
    export TEST_MODE=playback && pnpm test # this runs playback tests against the saved recordings.
    ```
    On Windows, you can use:

    ```shell
    pnpm turbo build --filter=${PACKAGE_NAME}... --token 1
    SET TEST_MODE=playback&& pnpm test # this runs playback tests against the saved recordings.
    ```
