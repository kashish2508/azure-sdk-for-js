...
Install the pinned `tsp-client` dependencies used by this repo:

```bash
npm ci --prefix eng/common/tsp-client
```

Then run the CLI with `npm exec --prefix eng/common/tsp-client --no -- tsp-client ...`. For more information, see the [TypeSpec Client Generator CLI documentation](https://github.com/Azure/azure-sdk-for-js/blob/main/eng/common/tsp-client/README.md).
...
{
  "scripts": {
    "generate:client": "echo use your package's existing generation script and invoke tsp-client through npm exec --prefix eng/common/tsp-client"
  }
}
```
...
# Run your package's existing generation script, or invoke tsp-client directly
...
1. **Generate and apply customizations:** Run your package's existing generation script, or invoke `tsp-client` directly via `npm exec --prefix ../../../eng/common/tsp-client --no -- tsp-client update`.
...
2. **Generate new code:** run your package's existing generation script or `npm exec --prefix ../../../eng/common/tsp-client --no -- tsp-client update`
3. **Build and test:** `pnpm turbo build --filter=@azure/your-package... --token 1` and then run the package's `pnpm test` script
